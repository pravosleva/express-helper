...
export const saveItem = (req: TModifiedRequest & {
  tg_chat_id: number;
  audits: NAuditList.TAudit[];
}, res: IResponse) => {
  ...
  try {
    ...
    switch (req.body.namespace) {
      case ENamespace.AUDIT_LIST: {
        req.subprojects.auxState.auditList.set(tg_chat_id, audits)
        return res.status(200).send({
          ok: true,
          message: 'Saved',
          audits: req.subprojects.auxState.auditList.get(tg_chat_id),
          // _service: {
