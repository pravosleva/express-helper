...
export const add = (req: IRequest, res: IResponse) => {
  const { room_id, ts, tsTarget, username, text } = req.body

  // TODO: Не удалось получить user из JWT
  // if (!req.user)

  const requiredParams = ['username', 'ts', 'tsTarget', 'room_id', 'text']
  const errs = []
  ...
  const newStateChunk = { [String(ts)]: { username, ts, tsTarget, text } }
  ...
  const newRoomNotifsStruct: TRoomNotifs = { data: { ...oldRoomNotifsStruct.data, [String(ts)]: { ts, tsTarget, username, text } }, tsUpdate: newTs }
